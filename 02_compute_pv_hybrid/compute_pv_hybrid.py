#!/usr/bin/env python
"""Step 02 — Ertel PV on the native HYBRID (sigma-pressure) grid of CESM2-LE.

Reads CAM history files (U,V,T on 32 hybrid model levels + PS) staged by step 01
(``01_download_cesm2/``), which embed the hybrid coefficients ``hyam/hybm/P0``,
reconstructs the TRUE pressure of each level

    p(k,j,i) = hyam(k) * P0 + hybm(k) * PS(j,i),

and computes the full 3-term Ertel PV on terrain-following σ levels via
``ertel_pv_sigma`` (see ``src/ertel_pv.py``). This is the correct treatment the
AWS-zarr path cannot do (LENS2 zarr drops the coefficients, exposing only the
NOMINAL ``lev`` = (hyam+hybm)*P0).

Output: a σ-coordinate PV netCDF carrying ``pv_sigma``, ``p_sigma`` (actual
pressure of each σ level), ``theta_sigma`` (θ on σ) and ``ps`` — everything step
03 (``03_interp_to_levels/``) needs to interpolate onto pressure or isentropic
surfaces. A σ validation plot is written alongside.
...
def main():
    ...
    ap.add_argument("--stage-dir", required=True, type=Path,
                    help="dir with staged native hybrid U/V/T (cam.h6) + PS (cam.h1)")
    ...
    # ---- hybrid-correct sigma PV (full 3-term formula; θ returned for step 03) ----
    ...
    # ---- write the sigma-PV netCDF (the hand-off to step 03) ----
    ...
    xr.Dataset(
        ...
        attrs={"member": args.member, "date": date_str,
               ...
               "source": "CESM2-LE GDEX d651056 native cam.h6 (via step 01)"},
    ).to_netcdf(out_nc)
    print(f"  wrote {out_nc}  (feed this to 03_interp_to_levels/interp_pv_to_levels.py)")

    # ---- sigma validation plot ----
    ...
    for ax, tgt in zip(axes, [0.85, 0.50, 0.25]):
        k = int(np.argmin(np.abs(sig - tgt)))
        d = pv_sigma[k]; valid = np.isfinite(d)
    ...
    fig.suptitle(f"CESM2-LE m{args.member:02d} Ertel PV — sigma (native hybrid, "
                 f"{date_str})", fontsize=13, y=0.99)
    plt.tight_layout()
